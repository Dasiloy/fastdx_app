import 'package:fastdx_app/providers/providers.dart';
import 'package:flutter/material.dart';

import 'package:fastdx_app/widgets/widgets.dart';
import 'package:fastdx_app/models/models.dart';
import 'package:fastdx_app/services/services.dart';
import 'package:fastdx_app/screens/vendor/reviews/reviews_screen.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';

class VendorReviews extends ConsumerWidget {
  const VendorReviews({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final asyncValue = ref.watch(
      reviewAggregateProvider(
        ReviewAggregateParams(resturantId: ref.read(appProvider).resturant?.id),
      ),
    );

    // dtat
    final data = asyncValue.asData?.value ?? ReviewAggregate();

    return AppCard(
      pt: 13,
      pl: 16,
      pb: 16,
      child: SizedBox(
        height: 94,
        width: double.infinity,
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            AppCardHeader.text(
              label: "Reviews",
              actionLabel: "See All Reviews",
              onPressAction: () {
                Navigator.of(context).push(
                  MaterialPageRoute(
                    builder: (_) {
                      return VendorReviewsScreen();
                    },
                  ),
                );
              },
            ),
            Spacer(),
            Row(
              children: [
                Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Icon(
                      Icons.star,
                      size: 26,
                      color: Theme.of(context).colorScheme.primary,
                    ),
                    SizedBox(width: 6),
                    Text(
                      data.formattedAverage,
                      style: Theme.of(context).textTheme.headlineMedium!
                          .copyWith(fontWeight: FontWeight.w700),
                    ),
                  ],
                ),
                const SizedBox(width: 12),
                Text(
                  "Total ${data.count} Reviews",
                  style: Theme.of(context).textTheme.bodyMedium,
                ),
              ],
            ),
          ],
        ),
      ),
    );
  }
}
